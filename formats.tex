...
\newcommand{\parttitle}{学培部-CAD口袋书}

\usepackage{fancyhdr}
% 恢复到之前的行为，但统一使用“奇数页样式”作为所有页的样式：
% 让 \chaptermark 和 \sectionmark 都写入 \leftmark（便于在只有 section 的文档中显示标题），
% 然后在页眉中间位置统一显示 \leftmark，页码显示在外侧角落。
...
\else
    \fancypagestyle{mystyle}{
        \fancyhf{}
        % 外侧页眉显示页码（LE = left on even pages, RO = right on odd pages）
        \fancyhead[LE,RO]{\pagenumberfont\thepage}
        % 无论奇偶页，中间都显示 \leftmark（即使用奇数页的样式）
        \fancyhead[CE]{\headfont\leftmark}
